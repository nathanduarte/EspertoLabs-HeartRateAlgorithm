...
void loop()
{
  ...
  if(current == 0){
    ...
    if(heartRateTracker >= RATE_SIZE){
      ...
      debug.print("heartRateAverage: ");
      debug.println(heartRateAverage);
    }
...
{
  double HR_firstValue = heartRate[0];
  debug.print("HR1: ");
  for(int i = 0; i < RATE_SIZE; i++){
    debug.print(heartRate[i]);
    debug.print(" ");
  }
  debug.println("");
...
//  debug.print("weightSum: ");
//  debug.println(weightSum); 
...
//    debug.print("segmentSignal: ");
    // Isolate segment
    for(int j = i; j < i + interval; j++){
      segmentSignal[j-i] = sig[j];
//      debug.print(segmentSignal[j-i]);
//      debug.print(" ");
    }
...
//  debug.println("rawFirstDiffs:");
//  for(int m = 0; m < firstDiffsCap; m++){
//    debug.println(rawFirstDiffs[m]);
//  }
//  debug.println("");
...
/*
 * 2 bugs left to fix
 * 1 - for some reason the first rate does not stay constant (heartRate[0])
 * 2 - for some reason the bound selection process is acting poorly
*/
